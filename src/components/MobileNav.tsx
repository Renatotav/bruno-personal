...
export default function MobileNav() {
  ...
  const drawer = (
    <div className="fixed inset-0 z-[9999] md:hidden">
      ...
      />
      {/* Drawer panel */}
      <div className="absolute left-0 top-0 bottom-0 w-72 flex flex-col bg-[#0d1117] border-r border-slate-800 shadow-2xl">
        {/* Header */}
            ...
            return (
              <Link
                ...
                className={`flex items-center space-x-3 rounded-lg px-4 py-3 text-sm font-medium transition-all ${
                  isActive
                    ? "bg-teal-900/40 text-teal-400 border-l-2 border-teal-500"
        ...
        <div className="shrink-0 p-4 border-t border-slate-800">
          <form action="/api/auth/logout" method="POST">
            <button
              type="submit"
              className="flex w-full items-center space-x-3 rounded-lg px-4 py-2.5 text-sm font-medium text-slate-400 hover:bg-slate-800/40 hover:text-red-400 transition-all"
            >
              <svg className="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
  ...
  return (
    <>
      {/* Hamburger button — fica no header */}
      <button
        onClick={() => setIsOpen(true)}
        className="flex h-9 w-9 items-center justify-center rounded-lg border border-slate-700 bg-slate-800/80 text-slate-400 hover:text-white md:hidden"
      ...
      {/* Drawer — renderizado no <body> via portal */}
