...
export default async function DashboardPage() {
  const [aulas, alunos, locais, configs] = await Promise.all([
  ...
  const lucroSemanal = receitaBruta - custoTotal;
  const lucroMensal = lucroSemanal * 4.33;
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <div className="rounded-xl border border-slate-800 bg-slate-900/20 p-5">
          <div className="flex items-center justify-between">
            <p className="text-xs font-semibold text-slate-500 uppercase">Receita/Semana</p>
          ...
          <p className="text-3xl font-bold text-emerald-400 mt-3">{formatBRL(receitaBruta)}</p>
          <p className="text-xs text-slate-500 mt-1">{aulas.length} aulas × semana</p>
        </div>

        <div className="rounded-xl border border-slate-800 bg-slate-900/20 p-5">
          <div className="flex items-center justify-between">
            <p className="text-xs font-semibold text-slate-500 uppercase">Custo Gasolina</p>
          ...
          <p className="text-3xl font-bold text-red-400 mt-3">{formatBRL(custoTotal)}</p>
          <p className="text-xs text-slate-500 mt-1">
            {receitaBruta > 0 ? `${((custoTotal / receitaBruta) * 100).toFixed(1)}% da receita` : "—"}
          </p>
        </div>

        <div className="rounded-xl border border-teal-800/30 bg-teal-950/10 p-5">
          <div className="flex items-center justify-between">
            <p className="text-xs font-semibold text-slate-500 uppercase">Lucro/Semana</p>
          ...
          <p className="text-3xl font-bold text-teal-400 mt-3">{formatBRL(lucroSemanal)}</p>
          <p className="text-xs text-slate-500 mt-1">após combustível + desgaste</p>
        </div>

        <div className="rounded-xl border border-slate-800 bg-slate-900/20 p-5">
          <div className="flex items-center justify-between">
            <p className="text-xs font-semibold text-slate-500 uppercase">Projeção Mensal</p>
            <span className="rounded-lg bg-slate-800/80 p-2 text-slate-400">
              <svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
              </svg>
            </span>
          </div>
          <p className="text-3xl font-bold text-white mt-3">{formatBRL(lucroMensal)}</p>
          <p className="text-xs text-slate-500 mt-1">
            {alunos} alunos · {locais} locais
          </p>
        </div>
      </div>
